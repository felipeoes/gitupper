...
router.register('bee-submissions',
                BeecrowdSubmissionsViewSet, basename='BeecrowdSubmissions')
router.register('beecrowd-users',
                BeecrowdUsersViewSet, basename='BeecrowdUsers')
router.register('temp-progress', TemporaryProgressesViewSet,
                basename='TempProgresses')
router.register('hacker-users', HackerUsersViewSet, basename='HackerUsers')
router.register('hacker-submissions', HackerSubmissionsViewSet,
                basename='HackerSubmissions')
router.register('leet-users', LeetUsersViewSet, basename='LeetUsers')
router.register('leet-submissions', LeetSubmissionsViewSet,
                basename='LeetSubmissions')
router.register('repo-events', RepoEventsViewSet, basename='RepoEvents')
router.register('repo-submissions', RepoSubmissionsViewSet,
                basename='RepoSubmissions')
...
urlpatterns = [
    ...
    path('fetch/submissions/', FetchSubmissionsView.as_view(), name='fetch_submissions'),
    path('unbind/platform/', PlatformUnbindView.as_view(), name='unbind_platform'),
    path('download/submissions/', SubmissionsDownloadView.as_view(),
         name='download_submissions'),
    path('api/v1/users/auth/reset-password/',
         ResetPasswordView.as_view(), name='reset_password'),
    path('api/v1/users/auth/validate-reset/',
         ValidateResetView.as_view(), name='validate_reset'),
    path('users/github/<int:github_id>/',
         ListUserByGithubIdSerializer.as_view()),
    path('users/submissions/beecrowd/<bee_id>/',
         ListUserBeeSubmissionsSerializer.as_view()),
    path('users/submissions/hackerrank/<hacker_id>/',
         ListUserHackerSubmissionsSerializer.as_view()),
    path('users/submissions/leetcode/<leet_id>/',
         ListUserLeetSubmissionsSerializer.as_view()),
    path('api/v1/users/auth/change-password/',
