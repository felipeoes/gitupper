...
def convert_datetime_pattern(
    input_datetime: Union[str, int], target=TARGET_DATETIME_FORMAT
) -> datetime:
    # TARGET: "%d/%m/%Y %H:%M:%S""
    # US: 1/2/23, 5:25 PM
    ...
    us = re.search(r"\d{1,2}/\d{1,2}/\d{2}, \d{1,2}:\d{1,2} [AP]M", input_datetime)
    br = re.search(r"\d{1,2}/\d{1,2}/\d{4} \d{1,2}:\d{1,2}:\d{1,2}", input_datetime)
...
def format_datetime(datetime: Union[str, int]) -> datetime:
    """Format datetime string or timestamp to datetime object"""
    try:
        return convert_datetime_pattern(datetime).replace(tzinfo=timezone.utc)
    except:
        return None
...
def check_submissions_offset(gitupper_id, platform_prefix):
    submission_tracker = SubmissionTracker.objects.filter(
        gitupper_user_id=gitupper_id, platform_prefix=platform_prefix
    ).first()

    if submission_tracker:
        return (
            submission_tracker.content_object.id,
            submission_tracker.content_object.date_submitted,
        )
    else:
        return 0, None
...
class BackgroundSubmissionsSaver(Thread):
    def run(self, *args, **kwargs):
        ...
        eval(platform).objects.bulk_create(
            [
                eval(platform)(
                    **{
                        **submission_obj,
        ...
        last_submission = max(submissions_objs, key=lambda x: x["date_submitted"])

        serialized_submission = eval(f"{platform_prefix.capitalize()}Submission")(
            **last_submission
        )
        save_submissions_offset(
            platform_user.gitupper_user.gitupper_id,
            platform_prefix,
